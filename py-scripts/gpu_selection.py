...
def get_sep_lines(nvidia_out: bytes) -> list[str]:
...
def get_gpu_numbers_mib(nvidia_out: bytes) -> dict[str, int]:
...
def get_command_for_process_id(process_id: int) -> str:
    command = system_call('ps -p ' + str(process_id) + ' -o args').strip().decode(
        'utf-8',
    ).replace('COMMAND', '').replace('\n', '')
    return command


def get_user_for_process_id(process_id: int) -> str:
    return system_call('ps -o user= -p ' + str(process_id)).strip().decode('utf-8')


def get_starttime_for_process_id(process_id: int) -> str:
    return system_call('ps -p ' + str(process_id) + ' -o lstart').strip().decode('utf-8').replace('STARTED', '').replace('\n', '')


def get_process_dict() -> dict[str, int | str]:
    nvidia_out = system_call('nvidia-smi')
    lines = str(nvidia_out).split('\\n')
...
def get_gpu_usage(
        gpu_dict: dict[str, str | int],
        process_dict: dict[str, str | int],
        gpu_name: str,
...
def plot_gpu_usage() -> None:
    ...
    fig, axs = plt.subplots(
        num_cols, num_rows, figsize=(num_rows*5, num_cols*5),
    )
    ...
    for n_r in range(num_rows):
        for n_c in range(num_cols):
            ...
            usage_dict, process_list_dict = get_gpu_usage(
                gpu_dict, process_dict, cur_gpu,
            )
            ...
            axs[n_c, n_r].pie(
                x=usages, autopct='%.1f%%',
                explode=explodes, labels=users, pctdistance=0.5,
            )
...
def print_gpu_usage() -> None:
    ...
    for cur_gpu in gpu_list:
        ...
        for i in range(len(sort_ids)):
            ...
            print(
                '        Processes: ' +
                str(process_list_dict[users[sort_ids[i]]]),
            )


def print_commands_for_user(username: str) -> None:
    process_dict = get_process_dict()
...
def main() -> int:
    ...
    print_gpu_usage()
    print_commands_for_user(args.user)
    return 0
